...
class EmployeeForm(FlaskForm):
    first_name = StringField('First name', [DataRequired(), validators.Length(min=4, max=15), validators.Regexp('^[a-zA-Z\- ]+$', message='First name must only contain alphabetic characters and hyphens')])
    last_name = StringField('Last name', [DataRequired(), validators.Length(min=4, max=30), validators.Regexp('^[a-zA-Z\- ]+$', message='Last name must only contain alphabetic characters hyphens')])
    email = EmailField('Email', [DataRequired(), validators.Length(min=8, max=30)])
