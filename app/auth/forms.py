...
class RegistrationForm(FlaskForm):
    ...
    email = EmailField('Email', [validators.Length(min=8, max=30)])
    first_name = StringField('First Name', [DataRequired(), validators.Length(min=4, max=15), validators.Regexp('^[A-Za-z]+$', message='First name must only contain alphabetic characters')])
